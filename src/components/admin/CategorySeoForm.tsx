  ...
  return (
    <form action={formAction}>
      ...
      <AdminSeoFields key={categoryId} initial={initialSeo} pageName={categoryLabel}>
        ...
        {state.saved && (
          <p className="mt-4 text-sm text-green-600">SEO 설정이 저장되었습니다.</p>
        )}
        <button
          type="submit"
          disabled={pending}
          className="mt-4 rounded-lg bg-neutral-900 px-4 py-2 text-sm text-white transition-opacity hover:opacity-90 disabled:opacity-50"
        >
          {pending ? "저장 중…" : "SEO 저장"}
        </button>
      </AdminSeoFields>
